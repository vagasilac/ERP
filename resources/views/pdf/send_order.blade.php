...
<div id="content">
    <h2 class="text-center">{{ trans('Comandă') }}</h2>
    <div class="data">
        <div class="left">
            <h4>{{ trans('Conform ofertei Nr.') }}: {{ $data['offer_received_number'] }}</h4>
            <h4>{{ trans('Data') }}: {{ $data['offer_received_date'] }}</h4>
        </div>
    </div>
    <div class="clearfix"></div>
    <div class="data">
            ...
            @php
                ...
                foreach ($data['materials'] as $mat) {
                    ...
                    if (isset($materials[$index])) {
                        ...
                        if ($data['preview']) {
                            foreach($mat->offer->offer_suppliers as $offer) {
                                if ($offer->supplier_id == $data['supplier']->id) {
                                    $materials[$index]['price'] = $offer->price;
                                    break;
                                }
                            }

                        }
                        else $materials[$index]['price'] = $mat->offer->offer_supplier_accepted->first()->price;
                    }
                }
                $k = 0;
            @endphp
