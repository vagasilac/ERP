...
@section('title') {{ trans('Adaugă rol') }} @endsection

@section('content')
    <div class="content-fluid">
        {!! Form::model(null, ['action' => 'RolesController@store', 'id' => 'saveForm', 'files' => true]) !!}
                                ...
                                @foreach ($permissions as $k => $permission)
                                    <div class="child-container marginL0">
                                        @if (!isset($permission['id']))
                                            ...
                                            @foreach ($permission as $l => $perm)
                                                ...
                                                @if (!isset($perm['id']))
                                                    <div class="child-container">
                                                        <h4>{{ trans($l) }}</h4>
                                                        @foreach ($perm as $i => $p)
                                                            {!! Form::checkbox('permission[]', $p['id'], false, ['class' => 'select'] ) !!}
                                                ...
                                                @else
                                                    {!! Form::checkbox('permission[]', $perm['id'], false, ['class' => 'select'] ) !!}
                                            ...
                                            @endforeach
                                        @else
                                            {!! Form::checkbox('permission[]', $permission['id'], false, ['class' => 'select'] ) !!}
                                ...
                                @foreach ($notification_types as $k => $notification_type)
                                    <div class="child-container marginL0">
                                        @if (!isset($notification_type['id']))
                                            ...
                                            @foreach ($notification_type as $l => $type)
                                                ...
                                                @if (!isset($type['id']))
                                                    <div class="child-container">
                                                        <h4>{{ trans($l) }}</h4>
                                                        @foreach ($type as $i => $t)
                                                            {!! Form::checkbox('notification_type[]', $t['id'], false, ['class' => 'select'] ) !!}
                                                ...
                                                @else
                                                    {!! Form::checkbox('notification_type[]', $type['id'], false, ['class' => 'select'] ) !!}
                                            ...
                                            @endforeach
                                        @else
                                            {!! Form::checkbox('notification_type[]', $notification_type['id'], false, ['class' => 'select'] ) !!}
