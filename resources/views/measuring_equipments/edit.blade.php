...
@section('content')
    <div class="content-fluid">
        {!! Form::model($item, ['action' => ['MeasuringEquipmentsController@update', $item->id], 'id' => 'saveForm', 'files' => true]) !!}
        <div class="page-header no-border col-xs-12">
            <h2>&nbsp;</h2>
            <div class="buttons-container">
                {!! Form::submit(trans('Salvează'), ['class' => 'btn btn-default']) !!}
                <a href="{{ action('MeasuringEquipmentsController@index') }}" class="btn btn-secondary marginL15 back-btn">{{ trans('Înapoi') }}</a>
            </div>
        </div>
        ...
        <div class="tab-content col-xs-12 marginT30">
            <div class="tab-pane active" id="general-container" role="tabpanel">
                <div class="col-xs-12 col-sm-6 marginR15">
                    ...
                    <div class="form-group @if ($errors->has('inventory_number')) has-error @endif">
                        ...
                        @if ($errors->has('inventory_number'))
                            <span class="help-block">{{ $errors->first('inventory_number') }}</span>
                        @endif
                    </div>
                    <div class="form-group">
                        {!! Form::label('Calibration Validity', trans('Valabilitatea calibrării'), ['class'=> 'control-label input-with-icon']) !!}
                        {!! Form::text('calibration_validity', null, ['class' => 'form-control has-datepicker']) !!}
                    </div>

                    <div class="form-group focused">
                        {!! Form::label('Measuring Range', trans('Domeniu de măsurare'), ['class'=> 'control-label small-label']) !!}
                    ...
                    <div class="form-group{{ $errors->has('photo') ? ' has-error' : '' }} row clearfix">
                        @if ($item->photo)
                            <div class="marginB15 col-xs-3" id="image-container">
                                <img class="marginB5 block img-responsive img-circle" src="{{ action('FilesController@image', base64_encode('ims/measuring_equipments/'.$item->id.'/photos/thumbnails/'.$item->photo)) }}" alt="{{ $item->photo }}" />
                            </div>
                        @endif
                        <div class="col-xs-9">
                            @if ($item->photo)
            ...
            <div class="tab-pane" id="documents-container" role="tabpanel">
                ...
                <div class="table-responsive">
                    @include('measuring_equipments._documents_list', ['documents' => $documents])
                </div>
            </div>
        </div>
...
@section('content-modals')
    <div class="modal fade" id="image-delete-modal">
        {!! Form::open(['action' => ['MeasuringEquipmentsController@destroy_photo', $item->id], 'method' => 'DELETE']) !!}
    ...
    <div class="modal fade" id="delete-modal">
        {!! Form::open(['action' => ['MeasuringEquipmentsController@documents_multiple_destroy', $item->id], 'method' => 'DELETE']) !!}
        <div class="modal-dialog modal-sm">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
    ...
    <script>
        jQuery(document).ready(function($) {
            ...
            $('.upload-btn').click(function() {
                setDropzoneUpload("{{ action('MeasuringEquipmentsController@documents_upload', [$item->id]) }}");
            });
