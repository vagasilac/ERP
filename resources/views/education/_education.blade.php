        ...
        @if (count($education) > 0)
            @foreach ($education as $k => $item)
                <tr>
                    ...
                    @endforeach
                    <td>@if ($is_user == true && !$item->user_confirmed(\Auth::user()->id)) <a class="btn btn-xs btn-success control-plan-submit" href="{{ action('EducationController@confirmed', $item->id) }}">{{ trans('Confirm') }}</a> @else {{ $item->get_user_confirmed_rating() }} @endif</td>
