...
@section('title') {{ trans('Standarde') }} <a href="{{ action('SettingsController@standards_create') }}" class="action pull-right"><i class="fa fa-plus"></i> {{ trans('Adaugă standard nou') }}</a> @endsection

@section('content')
    <div class="content-fluid">
        <div class="actions-bar clearfix" data-target="#standards-table">
        ...
        <div class="filters-bar clearfix" data-target="#standards-table">
            <div class="col-xs-12">
                <form action="" method="get" class="form-inline filters">
                    <div class="form-group marginR15">
                        <label class="control-label">{{ trans('Căutare standard') }}</label>
        ...
        <div class="table-responsive marginT30 list-container">
            @include('settings.standards._standards_list');
        </div>
    </div>
@endsection
...
@section('content-modals')
    <div class="modal fade" id="delete-modal">
        {!! Form::open(['action' => ['SettingsController@standards_multiple_destroy'], 'method' => 'DELETE']) !!}
        <div class="modal-dialog modal-sm">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title" >{{ trans('Șterge standard') }}</h4>
                </div>
                <div class="modal-body">
                    {{ trans('Doriți să ștergeți aceste standarde') }}
